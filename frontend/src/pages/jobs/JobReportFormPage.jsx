...
import { JOB_STATUS, UserRole } from '@/lib/constants'
...
/** A tappable phone number, for calling the customer or the site contact. */
...
export function JobReportFormPage() {
  ...
  useEffect(() => {
    if (!job) return
    // The person on site signs; the site contact is a better guess than the client.
    setSignatureName(
      (current) =>
  ...
  async function handleComplete() {
    ...
    try {
      ...
      if (signatures.customer && !fields.customer_unable_to_sign) {
        signaturePayload.customer_signature = signatures.customer
      }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 px-4 pb-16 pt-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <CallLink phone={job.customer_phone} />
            </div>
          </div>

          {job.site_contact_name || job.site_contact_phone ? (
            <div className="flex items-start gap-3">
              <UserRound className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground/70" />
              <div className="min-w-0">
                <p className="text-sm text-foreground">
                  <span className="text-muted-foreground">On site: </span>
                  {job.site_contact_name ?? 'Site contact'}
                </p>
                <CallLink phone={job.site_contact_phone} />
              </div>
            </div>
          ) : null}
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {job.order_number ? ` · Order no. ${job.order_number}` : ''}
          </p>
        </div>
      </Step>
      ...
      <Step
        number={3}
        title="Visit & pest activity"
        icon={Search}
        description="Tap what applies. Changes save automatically."
        muted={isPending}
      >
      ...
      <Step
        number={4}
        title="Findings"
        description="An overall summary, then what you found area by area, with photos."
        muted={isPending}
      >
      ...
      <Step
        ...
        description="What the customer needs to put right."
        muted={isPending}
      >
      ...
      <Step
        number={6}
        title="Action taken & products"
        icon={SprayCan}
        description="What you did, and every product you used."
        muted={isPending}
      >
        <div className="space-y-4">
      ...
      <Step
        number={8}
        title="Sign off"
        description="Tick the assessments, capture both signatures, then complete the job."
        muted={isPending}
      >
        <div className="space-y-6">
          <FormField label="Assessments in place">
