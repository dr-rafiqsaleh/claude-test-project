...
def main() -> None:
    ...
    try:
        with TestClient(app) as api:
            ...
            roles = api.get("/api/v1/roles", **admin).json()["data"]
            ...
            me = api.get("/api/v1/auth/me", **tom).json()["data"]
            ...
            assert api.get("/api/v1/quotes", **tom).status_code == 403
            denied = api.get("/api/v1/invoices", **tom)
            assert denied.status_code == 403 and "View invoices" in denied.json()["message"], denied.text
            assert api.post("/api/v1/bookings", **tom, json={}).status_code in (403, 422)
            assert api.get("/api/v1/roles", **office).status_code == 403
            assert api.put("/api/v1/settings", **office, json={"company_name": "X"}).status_code == 403
            ok(f"refusals say what's missing: '{denied.json()['message']}'")

            job = api.get(f"/api/v1/bookings/{booking.json()['data']['id']}", **tom).json()["data"]
            assert job["quoted_amount"] is None and job["internal_notes"] is None
            office_view = api.get(f"/api/v1/bookings/{job['id']}", **office).json()["data"]
            ...
            names = [person["full_name"] for person in api.get("/api/v1/bookings/technicians", **office).json()["data"]["items"]]
            assert "Sam Senior" in names and "Olive Office" not in names, names
            assert api.get("/api/v1/bookings", **sam).json()["data"]["total"] == 1
            assert api.get("/api/v1/bookings", **tom).json()["data"]["total"] == 1
            ok("a Senior Technician role: can be given jobs, sees every job, can't book them")

            api.put(f"/api/v1/users/{tom_id}", **admin, json={"role": senior_key})
            other = api.post(
                "/api/v1/bookings",
                **office,
                json={
            ...
            assert api.get("/api/v1/bookings", **tom).json()["data"]["total"] == 2
            ...
            api.put("/api/v1/roles/office_staff", **admin, json={"permissions": office_perms})
            assert api.get("/api/v1/invoices", **office).status_code == 403
            ok("taking 'View invoices' from Office Staff takes effect at once")

            assert api.put("/api/v1/roles/admin", **admin, json={"permissions": []}).status_code == 400
            assert api.delete("/api/v1/roles/technician", **admin).status_code == 400
            in_use = api.delete(f"/api/v1/roles/{senior_key}", **admin)
            ...
            spare = api.post("/api/v1/roles", **admin, json={"name": "Temp", "permissions": []}).json()["data"]
            assert api.delete(f"/api/v1/roles/{spare['key']}", **admin).status_code == 200
            assert api.post("/api/v1/users", **admin, json={
                "email": "x@example.com", "full_name": "X", "role": "no_such_role",
            }).status_code == 422
            ...
            seen = [b["id"] for b in api.get("/api/v1/bookings", **nina).json()["data"]["items"]]
            assert pool_id in seen and sams_id not in seen, seen
            assert api.get(f"/api/v1/bookings/{pool_id}", **nina).status_code == 200
            assert api.get(f"/api/v1/bookings/{sams_id}", **nina).status_code == 403
            ok("a job nobody is on shows to every technician; one given to someone else does not")

            assigned = api.put(f"/api/v1/bookings/{pool_id}", **office, json={"technician_id": sam_id})
            assert assigned.status_code == 200, assigned.text
            seen_after = [b["id"] for b in api.get("/api/v1/bookings", **nina).json()["data"]["items"]]
            assert pool_id not in seen_after, seen_after
            assert api.get(f"/api/v1/bookings/{pool_id}", **nina).status_code == 403
            ok("giving it to someone takes it off everyone else's list")

            print("Removing people")
            refused = api.delete(f"/api/v1/users/{tom_id}/permanent", **admin)
            ...
            assert api.delete(f"/api/v1/users/{spare_id}/permanent", **admin).status_code == 200
            assert api.get(f"/api/v1/users/{spare_id}", **admin).status_code == 404
            ok(f"someone with work on record is refused: '{refused.json()['message']}'")
            ok("someone with nothing on record is deleted for good")

            admin_id = api.get("/api/v1/auth/me", **admin).json()["data"]["id"]
            assert api.delete(f"/api/v1/users/{admin_id}/permanent", **admin).status_code == 400
            assert api.delete(f"/api/v1/users/{admin_id}", **admin).status_code == 400
