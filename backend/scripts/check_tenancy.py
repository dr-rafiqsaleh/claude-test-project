...
CHECK_DB = "pestbase_tenancy_check"
...
async def reset() -> None:
    ...
    # PestBase's own staff: no client of their own, and the admin role so they can
...
def main() -> None:
    ...
    try:
        with TestClient(app) as api:
            ...
            acme = api.post("/api/v1/clients", **platform, json={"name": "Acme Pest Control"})
            beta = api.post("/api/v1/clients", **platform, json={"name": "Beta Pest Services"})
            ...
            clash = api.post("/api/v1/clients", **platform, json={"name": "acme pest control"})
            ...
            for name, cid in (("Acme", acme_id), ("Beta", beta_id)):
                roles = api.get("/api/v1/roles", **inside(platform, cid)).json()["data"]["roles"]
            ...
            def add_admin(cid: str, email: str, name: str) -> str:
                response = api.post(
                    "/api/v1/users",
                    **inside(platform, cid),
                    json={"email": email, "full_name": name, "role": "admin"},
                )
            ...
            assert api.get("/api/v1/users", **ann).json()["data"]["total"] == 1
            assert api.get("/api/v1/users", **ben).json()["data"]["total"] == 1
            ...
            assert api.get("/api/v1/users", **inside(ann, beta_id)).json()["data"]["total"] == 1
            ...
            acme_list = api.get("/api/v1/customers", **ann).json()["data"]
            assert acme_list["total"] == 1, acme_list
            assert acme_list["items"][0]["first_name"] == "Alice"
            assert api.get(f"/api/v1/customers/{beta_customer}", **ann).status_code == 404
            assert api.get(f"/api/v1/customers/{acme_customer}", **ben).status_code == 404
            ...
            found = api.get("/api/v1/search", **ann, params={"q": "Bob"}).json()["data"]
            assert found["total_count"] == 0, found
            assert "Bob" not in str(found["results"]), found["results"]
            mine = api.get("/api/v1/search", **ann, params={"q": "Alice"}).json()["data"]
            ...
            perms = api.get("/api/v1/roles", **ann).json()["data"]["roles"]
            office = next(r for r in perms if r["key"] == "office_staff")
            trimmed = [p for p in office["permissions"] if p != "invoices.view"]
            assert api.put("/api/v1/roles/office_staff", **ann, json={"permissions": trimmed}).status_code == 200
            beta_office = next(
                r for r in api.get("/api/v1/roles", **ben).json()["data"]["roles"]
            ...
            assert api.put("/api/v1/settings", **ann, json={"company_name": "Acme Pest Control"}).status_code == 200
            beta_settings = api.get("/api/v1/settings", **ben).json()["data"]
            ...
            assert api.get("/api/v1/clients", **platform).status_code == 200
            mixed = api.get("/api/v1/customers", **platform)
            ...
            assert api.get("/api/v1/customers", **inside(platform, acme_id)).status_code == 200
            assert api.delete("/api/v1/support-access", **ann).status_code == 200
            shut = api.get("/api/v1/customers", **inside(platform, acme_id))
            assert shut.status_code == 403, shut.text
            assert "not opened their records" in shut.json()["message"], shut.json()
            ok(f"once the client closes access, PestBase is locked out: '{shut.json()['message'][:60]}...'")

            reopened = api.post("/api/v1/support-access", **ann, json={"hours": 4, "reason": "Numbering"})
            assert reopened.status_code == 201, reopened.text
            assert api.get("/api/v1/customers", **inside(platform, acme_id)).status_code == 200
            ok("and letting them back in works, for the hours the client chose")

            print("Suspending a client")
            assert api.put(f"/api/v1/clients/{beta_id}", **platform, json={"status": "suspended"}).status_code == 200
            # Asking for a sign-in code is refused for a suspended client, before
            # any email goes out. SuperTokens answers 200 with a GENERAL_ERROR
            # body rather than an HTTP error, which is its convention.
            blocked = api.post(
                "/auth/signinup/code",
                json={"email": "ben@beta.test"},
            )
            assert blocked.json().get("status") == "GENERAL_ERROR", blocked.text
            assert api.get("/api/v1/customers", **ben).status_code == 403
            assert api.get("/api/v1/customers", **ann).status_code == 200
            ok("a suspended client cannot sign in and its live sessions stop working; other clients carry on")

            assert api.put(f"/api/v1/clients/{beta_id}", **platform, json={"status": "active"}).status_code == 200
            resumed = api.post("/auth/signinup/code", json={"email": "ben@beta.test"})
            assert resumed.json().get("status") == "OK", resumed.text
            ok("switching it back on restores access")

            in_use = api.delete(f"/api/v1/clients/{beta_id}", **platform)
            assert in_use.status_code == 400 and "user" in in_use.json()["message"].lower(), in_use.text
            spare = api.post("/api/v1/clients", **platform, json={"name": "Empty Co"}).json()["data"]
            assert api.delete(f"/api/v1/clients/{spare['id']}", **platform).status_code == 200
